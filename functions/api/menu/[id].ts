...
export async function onRequestPut(context: any) {
  try {
    ...
    const calories = body.calories !== undefined ? (body.calories === '' || body.calories === null ? null : Number(body.calories)) : (existing.calories ?? null);
    const sortOrder = body.sortOrder !== undefined ? Number(body.sortOrder) : (existing.sort_order ?? 0);

    await execute(db,
      'UPDATE menu_items SET category_id = ?, name = ?, description = ?, price = ?, type = ?, image = ?, available = ?, popular = ?, calories = ?, sort_order = ?, updated_at = ? WHERE id = ?',
      categoryId, name, description || '', price, type || 'veg', image || '', available, popular, calories, sortOrder, now, id
    );

    return new Response(JSON.stringify(rowToItem({ ...existing, category_id: categoryId, name, description: description || '', price, type: type || 'veg', image: image || '', available, popular, calories, sort_order: sortOrder, updated_at: now })), { headers: CORS });
  } catch (error) {
